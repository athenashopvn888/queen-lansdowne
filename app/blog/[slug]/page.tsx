...
export async function generateMetadata({ params }: BlogPostPageProps): Promise<Metadata> {
  ...
  if (staticPost) {
    return {
      ...
      alternates: { canonical: `https://${STORE_BLOG_CONFIG.domain}/blog/${slug}` },
    };
  }
  const title = slug.split("-").map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join(" ");
  return {
    title: `${title} - Blog | ${STORE_BLOG_CONFIG.storeName}`,
    description: `Read store guides and local menu notes from ${STORE_BLOG_CONFIG.storeName}.`,
    alternates: { canonical: `https://${STORE_BLOG_CONFIG.domain}/blog/${slug}` },
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  const { slug } = await params;
  if (!getStaticPost(slug)) notFound();
